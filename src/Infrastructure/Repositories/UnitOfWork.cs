...
namespace ErpDashboard.Infrastructure.Repositories
{
    ...
    public class CustomUnitOfWork<TId> : ICustomIUnitOfWork<TId>
    {
        ...
        public CustomUnitOfWork(ERBSYSTEMContext dbContext, ICurrentUserService currentUserService, IAppCache cache)
        {
            _dbContext = dbContext ?? throw new ArgumentNullException(nameof(dbContext));
            _currentUserService = currentUserService;
            _cache = cache;
        ...
        public CustomIRepositoryAsync<TEntity, TId> Repository<TEntity>() where TEntity : class
        {
            if (_repositories == null)
                _repositories = new Hashtable();
            ...
            return (CustomIRepositoryAsync<TEntity, TId>)_repositories[type];
        }
